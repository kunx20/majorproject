...
- Python 3.10+
- pip or conda
- **Ollama** (for LLM-based answer generation) - Optional but recommended
...
This project now supports **Ollama** for advanced LLM-based answer generation using local models.

#### Install Ollama

1. **Download Ollama**: Visit [ollama.ai](https://ollama.ai) and download for your OS (Windows, macOS, Linux)

2. **Start Ollama Server:**
```bash
# On Windows/macOS/Linux, the Ollama app will start a background service
# Or run manually:
ollama serve
```
...
3. **Pull a Model:**
```bash
# Download Mistral (recommended, 7B model, ~4GB)
ollama pull mistral
...
```bash
# Should return a list of running models
curl http://localhost:11434/api/tags
```

#### How It Works

- **With LLM enabled (`USE_LLM=true`)**: Answers are generated by the local Ollama model using retrieved context
- **With LLM disabled (`USE_LLM=false`)**: Falls back to rule-based text simplification (original behavior)
- **Automatic fallback**: If Ollama is unavailable, the system automatically falls back to rule-based answers
...
All tests should pass with `pytest tests/ -v`.
...
Both `{filename}` values are generated by the preceding workflow steps, for
example `guideline.txt` for processing and `guideline_chunks.json` for
embedding. Configure `MAX_UPLOAD_BYTES` and `CORS_ORIGINS` in the environment
to change the upload limit and allowed browser origins.
